...
const AppLayout = ({children}) => {
    const {isClient, isUserLoading, whereToGo} = useAuth({
        middleware: 'client'
    });
        ...
        <MainLayout>
            <div className="xl:container max-sm:px-4 max-sm:pt-0 px-8 pt-8 mx-auto">
                <div className="grid grid-cols-4 max-lg:grid-cols-6 max-md:grid-cols-4 gap-x-8 grid-flow-col gap-4">
                    ...
                    <main
                        className="col-start-2 col-end-4 max-lg:col-start-3 max-lg:col-end-7 max-md:col-start-1 max-md:col-end-5">{children}</main>
