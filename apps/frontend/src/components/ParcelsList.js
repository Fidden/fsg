...
import Parcels from '@/components/parcels.json';

const ParcelsList = (order) => {
  const { data: orders, isLoading, error, mutate } = useSWR('/orders', defaultFetcher);
  console.log(orders);
  if (isLoading) {
  ...
  return (
    <div className="pb-40">
      <div className="flex flex-wrap justify-between items-center gap-3 border-1 border-primary-8 rounded-xl py-3 pl-4 pr-3">
      ...
      <div className="border-1 border-primary-8 mt-2 p-6 rounded-xl flex flex-col gap-2">
        {/* {storages?.map((storage) => (
          // <ParcelItem storage={storage} key={storage.id} />
          <p>Storage or Parcel</p>
        ))} */}
