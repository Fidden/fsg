              ...
              <Transition.Child
                ...
                <div className="fixed inset-0 overflow-y-auto">
                  <div className="flex min-h-full items-center justify-center py-8">
                    <Dialog.Panel>
                      <form onSubmit={handleSubmit(onSubmmit)} className="w-96 gap-8 flex flex-col p-8 rounded-xl bg-white">
                        <FormHead>Добавление посылки</FormHead>
                        <div className="flex flex-col gap-4 text-left">
                          <InputGroup label="Название посылки" errors={errors.name}>
                            <Input maxlength="20" {...register('name')} placeholder="Любое название" />
                            <InputDescription>Это поможет вам определить посылку</InputDescription>
                          </InputGroup>
                          <InputGroup label="Номер отслеживания (трекинг)" errors={errors.tracking_number}>
                            <Input {...register('tracking_number')} placeholder="1Z602AW60283834284" />
                            <InputDescription>Укажите трекинг с сайта, с которого сделан заказ</InputDescription>
                          </InputGroup>
                          <InputGroup label="Веб-сайт" errors={errors.shop}>
                            <Input {...register('shop')} placeholder="amazon.com" />
                            <InputDescription>Сайт, с которого сделан заказ</InputDescription>
                          </InputGroup>

                          <InputGroup label="Стоимость посылки" errors={errors.worth_amount}>
                            <div className="flex gap-2">
                              <Input type="number" {...register('worth_amount')} placeholder="10.25"/>
                              <FormSelect selectedCurrency={selectedCurrency}
                                          setSelectedCurrency={setSelectedCurrency}
                                          availableCurrencies={currencies}/>
                            </div>

                            <InputDescription>Укажите, за какую цену был приобретён товар</InputDescription>
                          </InputGroup>
                          <div className="p-4 bg-primary-2 gap-3 flex flex-col rounded-md">
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="flex flex-col gap-1">
                                <h6 className="font-medium text-md text-primary-100">Загрузка Invoice (чек)</h6>
                                <p className="text-md text-primary-60">
                                  Если стоимость посылки превышает 300 ₾, то вы обязательно должны загрузить Invoice. Если меньше 300 ₾, то
