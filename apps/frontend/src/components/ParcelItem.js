...
export default function ParcelItem({ order }) {
  ...
  const statusMap = {
    ...
    processing: <p className="text-sm text-accent_2-100 truncate">Прибыла на склад в {order.package.storage.country.name}</p>,
  ...
  return (
    <Disclosure>
      <Disclosure.Button className="w-full">
        <div className="flex items-center gap-3 px-4 max-sm:px-0 max-sm:hover:bg-white py-3 hover:bg-primary-2 cursor-pointer rounded-xl">
          ...
          <div className="flex gap-3 items-center grow truncate max-sm:flex-wrap max-sm:gap-1">
            ...
            <div className="flex flex-col text-right basis-2/4 max-sm:basis-full max-sm:text-left whitespace-nowrap min-w-0 max-sm:whitespace-normal truncate">
      ...
      <Disclosure.Panel className="pt-1 pb-4 px-4 max-sm:px-0">
        <table className="min-w-full table-auto rounded-xl text-primary-100 text-md divide-y divide-primary-8 bg-primary-2">
