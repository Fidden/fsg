...
export default function AccountMenu({ user }) {
  ...
  return (
    <Menu as="div" className="relative inline-block text-left">
      <Menu.Button className="relative h-16 max-sm:h-10 max-sm:w-10 max-sm:gap-0 max-sm:px-2 max-sm:hover:bg-white px-4 gap-3 flex items-center hover:bg-primary-2 transition ease-in-out-out rounded-xl text-left">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-primary-60 text-sm max-sm:hidden">
            <span>{t('balance')}:</span>{' '}
            <span className="font-medium text-accent_1-100">{intl.formatNumber(user?.balance || -1, { style: 'currency', currency: 'GEL' })}</span>
          </p>
        </div>
      ...
      <Transition
        ...
        <Menu.Items className="absolute right-0 mt-1 w-56 origin-top-right divide-y divide-gray-100  outline-none max-h-80 overflow-auto rounded-xl bg-white border-1 border-primary-8 text-md shadow-xl">
          {(() => {
            if (isClient === null) {
              return (
                <>
